/*
 * AWS nitro-enclave machine (aarch64)
 *
 * Copyright (c) 2026 Gil Bahat
 *
 * Derived from the x86 nitro-enclave machine:
...
/*
 * Parse the EIF image passed as -kernel, extract its kernel/initrd/cmdline to
 * temporary files, and rewrite the machine's kernel/initrd/cmdline to point at
 * them.  The extracted SHA384 hashes are stashed for PCR programming on reset.
 *
 * Unlike the x86 machine (which overrides the microvm x86_load_linux hook),
 * ARM 'virt' has no kernel-load hook, so we run this before calling the parent
 * machvirt_init(), which then loads the extracted kernel via arm_load_kernel().
 */
static void nitro_enclave_load_eif(NitroEnclaveMachineState *nems)
{
    Error *err = NULL;
    char *eif_kernel, *eif_initrd, *eif_cmdline;
    MachineState *machine = MACHINE(nems);
...
static void nitro_enclave_machine_state_init(MachineState *machine)
{
    ...
    nitro_enclave_load_eif(ne_state);

    ne_class->parent_init(machine);

    nitro_enclave_devices_init(ne_state);

    /* The extracted kernel/initrd have been loaded by machvirt_init(). */
...
static void nitro_enclave_machine_reset(MachineState *machine, ResetType type)
{
    ...
    /*
     * qemu_system_reset() calls mc->reset (us) *instead of*
     * qemu_devices_reset(), so whoever overrides mc->reset owns resetting the
     * devices/CPUs. On x86 the microvm parent reset does that; the ARM 'virt'
     * machine installs no MachineClass::reset (it has no parent_reset here), so
     * we must call qemu_devices_reset() ourselves or the CPU boot PC is never
     * set and the guest executes from address 0.
     */
    if (ne_class->parent_reset) {
...
static const char *nitro_enclave_get_default_cpu_type(const MachineState *ms)
{
    /*
     * Nitro Enclaves are 64-bit only; default to a Graviton-like core under
     * TCG and to the best available core otherwise.
     */
    return tcg_enabled() ? ARM_CPU_TYPE_NAME("neoverse-n1")
...
static void nitro_enclave_class_init(ObjectClass *oc, const void *data)
{
    ...
    mc->family = "nitro_enclave_aarch64";
    ...
    mc->get_default_cpu_type = nitro_enclave_get_default_cpu_type;
